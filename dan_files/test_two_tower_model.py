...
def test_output_dimensions(model, model_params, batch_size):
    """Test if output dimensions are correct for various batch sizes"""
    # Create dummy inputs
    query_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
    pos_answer_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
    neg_answer_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
    
    # Get outputs
    query_output, pos_output, neg_output = model(query_inputs, pos_answer_inputs, neg_answer_inputs)
    
    # Check output shapes
    assert query_output.shape == (batch_size, model_params['emb_dims'])
    assert pos_output.shape == (batch_size, model_params['emb_dims'])
    assert neg_output.shape == (batch_size, model_params['emb_dims'])

def test_embedding_freeze(model, model_params):
    ...
    # Create dummy inputs and perform a forward and backward pass
    batch_size = 8
    query_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
    pos_answer_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
    neg_answer_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
...
def test_output_values_range(model, model_params):
...
def test_different_tower_depths(model_params, query_layers, answer_layers):
...
#     # Create dummy inputs
#     query_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
#     answer_inputs = torch.randint(0, model_params['vocab_size'], (batch_size,))
    
#     # Get outputs
#     query_output, answer_output = model(query_inputs, answer_inputs)
    
#     # Check output shapes
#     assert query_output.shape == (batch_size, model_params['emb_dims'])
#     assert answer_output.shape == (batch_size, model_params['emb_dims'])
...
#     query_output, answer_output = model(query_inputs, answer_inputs)
#     loss = (query_output - answer_output).sum()
#     loss.backward()
